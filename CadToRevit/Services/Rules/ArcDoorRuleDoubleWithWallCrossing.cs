...
{
    /// <summary>
    /// R3D dedicated rule for double-arc doors with hard wall crossing.
    /// This rule intentionally follows the R3BD combined-opening geometry path.
    /// </summary>
    public sealed class ArcDoorRuleDoubleWithWallCrossing : IDoorCandidateRule
    {
        public string Name => "R3D";
        ...
        public IEnumerable<DoorCandidate> GenerateCandidates(List<CadSegment> doorSegments, DoorDetectSettings settings)
        {
            ...
            if (horizontalCandidates.Count >= 2)
            {
                ...
                if (dominantClosingDir != null)
                {
                    ...
                    if (leftFinal.Count > 0 && rightFinal.Count > 0)
                    {
                        XYZ leftInnerAnchor = AveragePoint(leftFinal);
                        XYZ rightInnerAnchor = AveragePoint(rightFinal);
            ...
            DoorCandidate candidate = new DoorCandidate
            {
                ...
                RuleSource = Name,
                SymbolFamilyKind = DoorSymbolFamilyKind.DoubleArcDoorWithWallCrossing,
                SegmentIds = segments.Select(x => x.SegmentId).Distinct().ToList(),
                WallDirHint = openingDir,
                WidthSource = "R3DCombinedOpening",
